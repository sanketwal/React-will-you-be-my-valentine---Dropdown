import React, { useState, useReducer, isValidElement } from "react";
...
function App() {
	...
	function handleCityChange(event) {
		setCity(stateName.city[event.target.value])
	}
		...
		<div id="main">
			<select id="state" onChange={handleChange}>
				{states.map((state, index) => {
					return <option value={index}>{state.name}</option>
				})}
			</select>
			...
			<div>
				<h1 id="state-name">{stateName.name}</h1>
				<h3 id="state-description">{stateName.description}</h3>
			</div>

			<div>
				<h1 id="city-name">{city.name}</h1>
				<h3 id="city-description">{city.description}</h3>
			</div>

			<div>
				<h1 id="landmark-name">{landmark.name}</h1>	
				<h3 id="landmark-description">{landmark.description}</h3>
			</div>
		</div>
	);
}
